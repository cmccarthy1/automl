 ...
 dict:$[typ=`fresh;
  {d:`aggcols`cols2use`params`xv`prf`scf`seed!
     (first cols x;1_cols x;
      .ml.fresh.params;.ml.xv.kfshuff[3;1];
      .ml.xv.fitpredict;`class`regr!(`.ml.accuracy;`.ml.mse);42);
   $[y~(::);d;
     99h=type y;
     $[min key[y]in key[d];
       d[key y]:value y;
...
i.runmodel:{[xv;x;y;a;l;f;s]
 s:$[type[s]in(101h;-7h);s;@[{"i"$x};s;'`$"type not convertable"]];
 if[(-7h~type s)&l~`sklearn;s:enlist[`random_state]!enlist s];
