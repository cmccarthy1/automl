...
runexample:{[tb;tgt;ftype;ptype;p]
  ...
  // Save down an autogenerated report based on results from the pipeline
  if[2=dict`saveopt;
  ...
  // additional information to be saved in the metadata
  ...
  // The full system metadata which can be saved to disk
  metadict:dict,hp,exmeta;
  if[dict[`saveopt]in 1 2;
    i.savemdl[dtdict;bm 1;expmdl;mdls];
    i.savemeta[metadict;dtdict]];
  }
...
newproc:{[t;fp]
  ...
  metadata:getmeta[.aml.path,"/Outputs/",fp,"/Config/metadata"];
  typ:metadata`typ;
  data:$[`normal=typ;
  ...
  $[(mp:metadata[`pylib])in `sklearn`keras;
    ...
    [model:$[mp~`sklearn;skload;krload]path,"/Outputs/",fp,"/Models/",string metadata[`best_model];
     model[`:predict;<]data];
...
savedefault:{[fn;ftype]
  ...
  h:hopen hsym`$raze[path],"/code/mdl_def/",fn;
