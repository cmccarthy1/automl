...
post.report:{[dict;dt;fname;ptype]
 pdf:canvas[`:Canvas][fname,"q_automl_report_",ssr[sv["_";string(first[key[dict`model_scores]];dt`sttime)],".pdf";":";"."]];
 ...
 font[pdf;"Helvetica";11];
 f:cell[pdf;f;30;"Best scoring model = ",string first key[dict`model_scores]];
 ...
 if[not (first key[dict`model_scores])in i.excludelist;
   font[pdf;"Helvetica-Bold";13];
   gstitle:"Grid search for a ",(string first key[dict`model_scores])," model.";
